...
describe('GET /post/search', () => {
  it('Should send query keyword and return Post data', (done) => {
    api.get('/post/search?keyword=qq')
      .expect(200)
      .end((err, res) => {
        if (err) {
          done(err);
  ...
  it('Should send empty query and return 400', (done) => {
    api.get('/post/search?keyword=')
      .expect(400)
      .end((err, res) => {
        if (err) {
          done(err);
